// Function to select random questions from the array
  function getRandomQuestions(questionsArray, count) {
    const shuffledArray = questionsArray.sort(() => 0.5 - Math.random()); // Shuffle the questions array
    return shuffledArray.slice(0, count); // Return a subset of 'count' number of questions
  }
  
  // Selecting 15 random questions from the array
  ...
  // getting questions and options from the selected array
  ...
  function optionSelected(answer) {
    ...
    if (userAns == correcAns) { //if user selected option is equal to array's correct answer
      userScore += 1; //increment the score value with 1
